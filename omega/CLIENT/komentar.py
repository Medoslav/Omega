...
def run_co(s,jm,cl_id):
    """
    Otevře okno na vytvoření komentáře
    ...
    text_entry = tk.Text(text_window, font=("Arial", 12), width=50, height=10)
    ...
    def show_text():
        """
        Reaguje na stisk tlačítka Odeslat, zkonroluje jestli pole naní prázdné a odešle se na server
        :return:
        """
        ...
        x = client_metody.send_kom(s,jm,cl_id,text)
        if x == "Y":
            error.err("Komentář odeslán můžete zavřít okno.")
        if x == "N":
            error.err("Tento komentář je opsanej, nekraď cizí práci!!!")
